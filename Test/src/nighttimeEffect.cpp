...
NightTimeEffect::NightTimeEffect()
{
	Effect::Init(m_shaderProg);
	Init();
}

bool NightTimeEffect::Init()
{
	std::string vs = "shaders/" + GetGLVersionString() + "/shader.vs";
	std::string fs = "shaders/" + GetGLVersionString() + "/nighttime.fs";
