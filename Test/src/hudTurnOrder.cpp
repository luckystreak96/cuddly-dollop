...
HudTurnOrder::HudTurnOrder(std::pair<Fighter_ptr, Actor_ptr> owner)
{
	m_mustDisplay = false;
	_actor = owner;

	m_prevActorPos = _actor.second->GetPosRef();

	m_observedOrder = _actor.first->_OrderPosition;
...
{
	Vector3f pos = _actor.second->GetPosRef();
...
{
	if (!m_mustDisplay || _actor.first->Dead)
	{
